#!/bin/bash
while true; do
  n=$(find /mnt/task_runtime/Depthtrack_workspace/results/flextrackv2_v56_depthtrack_miss -iname '*.txt' 2>/dev/null | wc -l)
  echo "progress: $n/50"
  if [ "$n" -ge 50 ]; then break; fi
  sleep 30
done
echo "=== Miss complete, restoring list and running vot analysis ==="
...
if m:
    print(f'RESULT miss(V8-params): Precision={float(m.group(1))*100:.2f}% Recall={float(m.group(2))*100:.2f}% EAO={float(m.group(3))*100:.2f}%')
EOF
echo "=== V8PARAMS_MISS_ANALYSIS_DONE ==="
