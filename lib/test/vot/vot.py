...
try:
    import trax
    try:
        from trax.internal import trax_version
        version = trax_version()
    except (ImportError, AttributeError):
        version = b"4.0.0"
    if version.decode("ascii") < "4.0.0" and _USE_TRAX:
...
class VOT(object):
    ...
    def __init__(self, region_format, channels=None, multiobject: bool = None):
        ...
        if _USE_TRAX:
            ...
            kwargs = {}
            import inspect
            sig = inspect.signature(trax.Server.__init__)
            if 'customMetadata' in sig.parameters:
                kwargs['customMetadata'] = dict(vot="python")
            elif 'metadata' in sig.parameters:
                kwargs['metadata'] = dict(vot="python")
            if 'multiobject' in sig.parameters:
                kwargs['multiobject'] = multiobject
            self._trax = trax.Server([region_format], ["path"], channels, **kwargs)
            ...
            if hasattr(request, 'objects') and request.objects is not None:
    ...
    def report(self, status):
        ...
        if not _USE_TRAX:
            ...
            if not self._multiobject:
                import inspect
                sig = inspect.signature(self._trax.status)
                if len(sig.parameters) >= 2 and 'properties' in sig.parameters:
                    first_param = list(sig.parameters.values())[0]
                    if first_param.name == 'region':
                        self._trax.status(convert_trax(status), {})
                    else:
                        self._trax.status([(convert_trax(status), {})], {})
                else:
                    self._trax.status(convert_trax(status))
                ...
                self._trax.status([(convert_trax(x), {}) for x in status], {})
