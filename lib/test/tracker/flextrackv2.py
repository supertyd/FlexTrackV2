...
from lib.models.flextrackv2 import build_flextrackv2
...
class FlexTrackV2Tracker(BaseTracker):
    def __init__(self, params, dataset_name):
        super(FlexTrackV2Tracker, self).__init__(params)
        network = build_flextrackv2(params.cfg)
        ...
        # Ratio-sweep names ("<BASE>_MISSR###", e.g. RGBT234_MISSR050) are a
        # missing-RATE variant of the full benchmark. Resolve their online-update
        # thresholds to the BASE dataset's, so the degradation sweep runs at the
        # SAME operating point as the reported full-modality results instead of
        # silently falling back to DEFAULT (which disables online update).
        # Exact "_MISS" is left untouched -- it has its own tuned config key.
        import re as _re
        _mr = _re.search(r'_MISSRGB\d{3}$', DATASET_NAME) or _re.search(r'_MISSR\d{3}$', DATASET_NAME)
        if _mr:
    ...
    def initialize(self, image, info: dict):
        ...
        self.missing_list = [[1,1]]*self.num_template
        ...
        self.memory_missing_list = self.missing_list.copy()


    def track(self, image, missing: list = None, info: dict = None, ):
        H, W, _ = image.shape
        self.frame_id += 1
        ...
        # run the encoder
        with torch.no_grad():
            enc_opt,loss = self.network.forward_encoder(self.template_list, search_list, self.template_anno_list, missing=self.missing_list, current_missing=missing)
        ...
        # run the decoder
        with torch.no_grad():
            out_dict = self.network.forward_decoder(feature=out_neck,loss=loss)
        ...
        if (self.frame_id % self.update_intervals == 0):
            ...
            for i in range(1, self.num_template):
                ...
                self.missing_list.append(self.memory_missing_list[idx])
                self.missing_list.pop(1)
